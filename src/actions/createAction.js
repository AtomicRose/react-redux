...
const createAction = function (actionCreator) {
  if (!__PROD__) {
    if (typeof actionCreator !== 'function') console.log('Action creator should be a function');
  ...
  function dispatchAction(...args) {
    ...
    if (!defaultStore) console.log('Store is not initialized.');
  ...
  let actionType = __PROD__ ? String(actionId++) : `${actionId++}:${actionCreator.actionType || actionCreator.name}`;
